...
case='example_scenairo'
#case='example_scenairo_death'
list_temp=[]
if case=='example_scenairo':
    minmial_cap = 60
    maxmal_cap = 240
    path_files = '/Users/lucinezhong/Documents/LuZHONGResearch/20200720COVID-Controllers/results_scenarios/'
    df_temp=pd.read_csv(path_files+'best_case_sigmoid-vaccination-360.csv')
    df_temp=df_temp[df_temp['vaccination']==0.9]
    MSA_ids_input = pd.unique(df_temp['MSA_code'])
    dict_temp = dict(zip(df_temp['MSA_code'], df_temp['days']))
    dictionary = {}
    for msa in MSA_ids:
        dictionary[msa] = {}
        if msa in MSA_ids_input:
            dictionary[msa]['pop'] = MSA_statistics['pop'][msa]
            dictionary[msa]['days'] =(dict_temp[msa]+1)
            if dictionary[msa]['days']>maxmal_cap:
                dictionary[msa]['days']=maxmal_cap
            if dictionary[msa]['days']<minmial_cap:
                dictionary[msa]['days']=minmial_cap
            list_temp.append(dictionary[msa]['days'])
        else:
            dictionary[msa]['pop'] = 1
            dictionary[msa]['days']= -1


if case=='example_scenairo_death':
    minmial_cap = int(0/100)
    maxmal_cap = int(50000/100)
    path_files = '/Users/lucinezhong/Documents/LuZHONGResearch/20200720COVID-Controllers/results_scenarios/'
    df_temp=pd.read_csv(path_files+'best_case_sigmoid-vaccination-360.csv')
    df_temp=df_temp[df_temp['vaccination']==0.9]
    MSA_ids_input = pd.unique(df_temp['MSA_code'])
    dict_temp = dict(zip(df_temp['MSA_code'], df_temp['accumulated D']/100))
    dictionary = {}
    for msa in MSA_ids:
        dictionary[msa] = {}
        if msa in MSA_ids_input:
            dictionary[msa]['pop'] = MSA_statistics['pop'][msa]
            dictionary[msa]['days'] =int(dict_temp[msa])
            if dictionary[msa]['days']>maxmal_cap:
                dictionary[msa]['days']=maxmal_cap
            if dictionary[msa]['days']<minmial_cap:
                dictionary[msa]['days']=minmial_cap
...
'''
densitygrid=np.load('/Users/lucinezhong/Documents/LuZHONGResearch/20200720COVID-Controllers/results_trajectory_fitting/MSA_densitygrid.npy')
...
print('start_3')
cmap = cm.get_cmap('RdBu', maxmal_cap+1-minmial_cap)
colors=[cmap(i) for i in range(cmap.N)]
colors_dict=dict(zip([i for i in range(minmial_cap,maxmal_cap+1)],colors))
print(colors_dict)
...
for strx in ['days']:
    ...
    for msa in MSA_ids:
        if dictionary[msa][strx]==-1:
            ...
            if dictionary[msa][strx]<=minmial_cap:
                dictionary[msa][strx] = minmial_cap
            else:
                color = colors_dict[dictionary[msa][strx]]
            print(msa, dictionary[msa][strx])
        ...
        axes.fill(*morphed_border_st[msa].T,facecolor=color)
    ...
    dd = int((maxmal_cap-minmial_cap)/20)
    if dd==0:
        dd=1
    ...
    cax.yaxis.tick_right()
    #cax.set_yticklabels([cc for cc in np.arange(0, maxmal_cap, dd)], [str(cc) for cc in np.arange(0, maxmal_cap, dd)])
    ax2 = cax.twiny()
    cax.set_xticks([]), ax2.set_xticks([])
    #cax.set_ylim([0, 1])
    plt.savefig(path_files+'MSA_map_' + case + '.png',dpi=600)
    plt.close()
